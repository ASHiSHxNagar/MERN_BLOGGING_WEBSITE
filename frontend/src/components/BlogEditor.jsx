...
const BlogEditor = () => {
  ...
  useEffect(() => {
    if (!textEditor.isReady) {
      const editor = new EditorJS({
        ...
        onReady: () => {
          setTextEditor({ instance: editor, isReady: true });
        },
      });
    }
  ...
  const handlePublishEvent = () => {
    ...
    if (textEditor.isReady) {
      textEditor.instance
        .save()
        .then((data) => {
          const { blocks } = data; // Extract only the blocks
          if (blocks.length) {
            setBlog({ ...blog, content: blocks }); // Now storing only the blocks array
  ...
  const handleSaveDraft = async (e) => {
    ...
    try {
      if (textEditor?.isReady) {
        const content = await textEditor.instance.save();
        const blogObj = { ...blog, content: content.blocks, draft: true }; // Here you're storing only blocks
